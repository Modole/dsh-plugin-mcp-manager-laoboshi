...
export const name = "laobos-mcp-manager";
const FEATURE = "mcp";
...
export function apply(ctx, config) {
  // Feature bundles stay independent from the 劳博士 product shell.
  ...
  if (FEATURE === "mcp") mcp.initialize();
  ctx.effect(
    () => () => FEATURE === "mcp" ? mcp.close() : undefined,
    "laobos-system-tools: MCP connections",
  );
  if (FEATURE === "system-prompt") ctx.systemPrompt.section({
  ...
  if (FEATURE === "knowledge") ctx.systemPrompt.section({
...
不要保存临时对话、猜测、未经确认的结论、凭据或其他秘密。更新前先读取现有文档并携带 revision。删除必须使用 knowledge_delete。`,
  ...
  if (FEATURE === "workflow") ctx.systemPrompt.section({
...
更新、发布或启停前先读取当前工作流并携带 revision。删除必须使用 workflow_delete，并说明原因。`,
  ...
  if (FEATURE === "git") ctx.systemPrompt.section({
...
远端同步必须使用 git_publish；恢复文件或删除分支必须使用 git_restore。后两者会请求用户确认，拒绝后不得绕过。`,
  ...
  if (FEATURE === "knowledge") {
    ctx.tools.register(defineTool({
      ...
      execute: async (args) => {
        ...
        if (FEATURE === "knowledge" || FEATURE === "workflow") refreshDynamicTools();
  ...
  ctx.effect(() => ctx.webServer.register({
    kind: "prefix",
    path: "/laobos/api/mcp-manager",
    async handler(req, res) {
      try {
        assertLoopback(req);
        const url = new URL(req.url || "/", "http://127.0.0.1");
        const route = url.pathname.slice("/laobos/api/mcp-manager".length) || "/";
        let value;
        ...
        if (req.method === "GET" && route === "/brand/icon.png") {
          ...
          const manifest = Buffer.from(JSON.stringify({
            ...
            display: "standalone",
            icons: [{
              src: "/laobos/api/mcp-manager/brand/icon.png",
              sizes: "1024x1024",
              type: "image/png",
